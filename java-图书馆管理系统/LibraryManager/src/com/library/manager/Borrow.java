...
public class Borrow extends Frame {
	...
	Button querybtn = new Button("查询"), borrowbtn = new Button("借出"), closebtn = new Button("清除");
	String SepLine = "--------------------------------------------------";
	String[] sep = { "图书信息", "读者信息", "借阅信息" };
	Label[] seplabel = new Label[3];
	String[] optionname = { "书名：", "作者：", "出版社：", "出版时间：", "定价：", "存量：", "姓名：", "类型：", "可借数：", "可借天数：", "已借数量：",
			"是否可借：", "借阅日期：" };
	...
	public void queryActionPerformed(ActionEvent e) {
		...
		if (IfBorrowBack.findBook(bookid, readerid)) {
			...
			JOptionPane.showMessageDialog(null,
					book.getBookname() + "已被" + reader.getReadername() + "借阅" + fulldays + "天，且还未归还，该读者不能借阅此图书");
			setInitialize();
			return;
		...
		if (book != null && reader != null) {
			if (reader.getMax_num() <= Integer.parseInt(getReaderBorrowBookCounts(readerid))) {
				JOptionPane.showMessageDialog(null, "对不起该读者借阅图书太多，不能再借阅");
				return;
			}

			if (Integer.parseInt(getImposeRestrictionsOnDays_num(readerid)) > reader.getDays_num()) {
				JOptionPane.showMessageDialog(null,
						"对不起" + reader.getReadername() + "借阅天数已超过" + reader.getDays_num() + "天，不能再借阅");
				return;
			}
			if (book.getStock() == 0) {
	...
	public void borrowActionPerformed(ActionEvent e) {
		...
		if (!bookid.equals("") && !readerid.equals("")) {
			...
			String sql = "insert into borrow(id,book_id,reader_id,borrow_date,if_back)  values('" + IOL + "','" + bookid
					+ "','" + readerid + "','" + borrowdate + "','否')";
			String sql1 = "update book set stock='" + (Integer.parseInt(showoption[5].getText()) - 1) + "'"
					+ "where id='" + bookid + "'";
			int success = SqlCon.executeUpdate(sql);
			if (success == 1) {
				SqlCon.executeUpdate(sql1);
	...
	public String getImposeRestrictionsOnDays_num(String readerid) {// 读者借阅最早图书未归还天数；
		...
		Date date = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		sql = "select * from borrow where reader_id='" + readerid + "' and if_back='否'";
		...
		try {
			...
			a = 0;
			ResultSet rs1 = SqlCon.executeQuery(sql);
			while (rs1.next()) {
				sqldate[a] = sdf.parse(rs1.getString("borrow_date"));
				bookborrowdays[a] = Integer.parseInt(getReaderBorrowDays(sqldate[a], date));
				a++;
			}
			int temp = -1;
