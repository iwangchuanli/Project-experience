...
public class Panel extends JPanel {// 672是棋盘上能成五子的数目
	private URL blackImgURL = Game.class.getResource("e://temp//green.png");
	private ImageIcon black = new ImageIcon(blackImgURL);
	private URL whiteImgURL = Game.class.getResource("e://temp//red.png");
	private ImageIcon white = new ImageIcon(whiteImgURL);
	private URL currentImgURL = Game.class.getResource("e://temp//new_red.png");
				...
				for (j = 0; j <= 15; j++) { // 如果board元素值为0，则该坐标处为黑子
					if (this.board[i][j] == 0) {
						g.drawImage(black.getImage(), i * 30 + 31, j * 30 + 31,
								black.getImage().getWidth(black.getImageObserver()) - 3,
								black.getImage().getHeight(black.getImageObserver()) - 3, black.getImageObserver());
					}
					// 如果board元素值为1，则该坐标处为白子
					if (this.board[i][j] == 1) {
						g.drawImage(white.getImage(), i * 30 + 31, j * 30 + 31,
								white.getImage().getWidth(white.getImageObserver()) - 3,
								white.getImage().getHeight(white.getImageObserver()) - 3, white.getImageObserver());
					}
				}
			// 画出白子（电脑）当前所下子，便于辨认
			if (this.board[m][n] != 2)
				g.drawImage(current.getImage(), m * 30 + 31, n * 30 + 31,
						current.getImage().getWidth(current.getImageObserver()) - 4,
						current.getImage().getHeight(current.getImageObserver()) - 4, current.getImageObserver());
			// 判断输赢情况
			// 人赢
			if (this.pwin)
