...
public class QueryScoreByYearIFrame extends JInternalFrame {
private static final long serialVersionUID = -2378866877251982599L;
    ...
    public QueryScoreByYearIFrame() {   //构造器
     super();
     ...
     panel.add(panelOfQueryInput, BorderLayout.NORTH);
        yearChoice = new JComboBox();                      //查询条件选择框
     String[] array={"2015","2016","2017","2018"};      //设置选择框的内容
     for(int i=0;i<array.length;i++){
   yearChoice.addItem(array[i]);
   }
     panelOfQueryInput.add(new JLabel("请输入查询年份:")); //加入输入提示信息和输入选择框
     ...
     exitButton.addActionListener(new ActionListener(){      //为退出按钮添加监听器      
      public void actionPerformed(ActionEvent arg0) {
       QueryScoreByYearIFrame.this.setVisible(false);
        }});
   ...
   s[i][7] = new Double(book.score);
    ...
    class QueryButtonListener implements ActionListener {        //查询按钮的监听器
        public void actionPerformed(ActionEvent arg0) {
         ...
         tableBody=getObject(Student.getYearScoreList(year)); 
