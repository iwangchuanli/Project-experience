...
def cnblogsSpider(index):
    ...
    for i in range(len(titles)):
        insertsql = "INSERT INTO `original_php_article` (`title`, `intro`, `url`, `from`, `other`, `stock_time`) " \
                    "VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % \
...
def titleandintro():
    db = sql_operation.getcon()
    selectsql = "SELECT article_id,title,intro FROM original_php_article WHERE title_flag IS NULL or title_flag = 0"
    ...
    for row in results:
        ...
        if baidudetector.url(title) and baidudetector.url(intro):
            updatesql = "UPDATE `original_php_article` SET `title_flag`='0' WHERE (`article_id`='%d')" % (article_id)
        else:
            updatesql = "UPDATE `original_php_article` SET `title_flag`='1' WHERE (`article_id`='%d')" % (article_id)
...
def content():
    db = sql_operation.getcon()
    selectsql = "SELECT article_id,url FROM original_php_article WHERE (content_flag IS NULL  or content_flag = 0) and title_flag = 0"
    results = sql_operation.baseselect(db, selectsql)
    for row in results:
        article_id = row[0]
        url = row[1]
        ...
        if circleCheck(url):
            updatesql = "UPDATE `original_php_article` SET `content_flag`='0' WHERE (`article_id`='%d')" % (article_id)
        else:
            updatesql = "UPDATE `original_php_article` SET `content_flag`='1' WHERE (`article_id`='%d')" % (article_id)
        sql_operation.baseoperation(db, updatesql)
    sql_operation.closecon(db)
...
def sql2txt():
    db = sql_operation.getcon()
    selectsql = "SELECT title,url FROM original_php_article WHERE title_flag = 0 and content_flag = 0"
    results = sql_operation.baseselect(db, selectsql)
    fo = open("d://php.txt", "w", encoding='utf-8')
    for row in results:
        txt = row[0] + "\n" + row[1]+"\n\n"
        fo.write(txt)
    fo.close()
