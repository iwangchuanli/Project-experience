...
public class ReaderDelete extends Frame {
	...
	public static void updateActionPerformed(ActionEvent e) {
		String id = readertxt[0].getText(), readername = readertxt[1].getText(),
				readetype = readertype.getSelectedItem().toString();
		...
		try {
			...
			int b = SqlCon.executeUpdate(sql);
			if (b == 1) {
