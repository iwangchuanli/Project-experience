...
public class BookUpdate extends Frame {
	...
	public void savebtnActionPerformed() {
		...
		try {
			...
			String sql = "update book set bookname='" + inputstring[1] + "',booktype='" + typestr + "',author='"
					+ inputstring[3];
			sql = sql + "',translator='" + inputstring[4] + "',publisher='" + inputstring[5] + "',publish_time='"
					+ inputstring[6];
			sql = sql + "',price='" + price + "',stock='" + stock + "',page='" + page + "' where id='" + id + "'";

			int i = SqlCon.executeUpdate(sql);
			if (i == 1) {
